...
# --- ОБНОВЛЕННЫЙ ПРОМПТ STEP 1 (Определение Эмоционального Ядра + Нацеленность на Интенсивность) ---
PROMPT_STEP1 = """
...
JSON:
"""
# --- КОНЕЦ ОБНОВЛЕННОГО ПРОМПТА STEP 1 ---

# --- ОБНОВЛЕННЫЙ ПРОМПТ STEP 2 (Выбор Креативного Драйвера + Нацеленность на Интенсивность) ---
PROMPT_STEP2 = """
...
JSON:
"""
# --- КОНЕЦ ОБНОВЛЕННОГО ПРОМПТА STEP 2 ---

# --- ОБНОВЛЕННЫЙ ПРОМПТ STEP 3 (Выбор Эстетики и Ключевых Слов + Нацеленность на Интенсивность) ---
PROMPT_STEP3 = """
...
JSON:
"""
# --- КОНЕЦ ОБНОВЛЕННОГО ПРОМПТА STEP 3 ---
...
def run_iid_pipeline(input_json_path: Path, config_dir: Path):
    ...
    if not has_existing_prompts:
        ...
        # Используем обновленный PROMPT_STEP1
        ...
        # Используем обновленный PROMPT_STEP2
        ...
        # Используем обновленный PROMPT_STEP3
        ...
        try:
            ...
            creative_output_payload = {
                "creative_brief": creative_brief, # Теперь будет содержать выбранный ИИ стиль
