...
class ContentGenerator:
    ...
    def generate_sarcastic_comment(self, text, content_data):
        ...
        if "theme" in content_data and content_data["theme"] == "tragic":
            prompt_template = self.config['SARCASM'].get('tragic_comment_prompt', self.config['SARCASM']['comment_prompt'])
    ...
    def generate_interactive_poll(self, text, content_data):
        ...
        if "theme" in content_data and content_data["theme"] == "tragic":
            prompt_template = self.config['SARCASM'].get('tragic_question_prompt', self.config['SARCASM']['question_prompt'])
    ...
    def critique_content(self, content):
        ...
        try:
            ...
            critique = self.request_openai(prompt, self.config.get('CONTENT.critique.max_tokens', 200), self.temperature)
    ...
    def run(self):
        ...
        try:
            ...
            text = self.generate_text(topic, content_data)
