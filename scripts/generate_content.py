...
def save_to_b2(folder, content):
    ...
    try:
        ...
        # ✅ Проверяем корректность структуры JSON перед сохранением
        if not isinstance(content, dict):
            logger.error("❌ Ошибка: Контент должен быть словарём!")
            return

        # ✅ Проверяем, что sarcasm передан отдельно
        ...
        # 🔹 Добавлено: Проверка и исправление poll
        ...
        content["sarcasm"] = sarcasm_data  # Обновляем исправленный sarcasm

        # ✅ Загружаем JSON в B2 без двойного кодирования
...
class ContentGenerator:
    ...
    def generate_interactive_poll(self, text):
        """
        Генерирует интерактивный саркастический вопрос в формате JSON.
        """
        if not self.config.get('SARCASM.enabled', False):
            self.logger.info("🔕 Сарказм отключён в конфигурации.")
            return {}
