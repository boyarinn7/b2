...
folders = ["444/", "555/", "666/"]
file_extensions = ["-image.png", "-metadata.json", "-video.mp4"]
...
def upload_mock_files(folder, group_id):
    """Загружает файлы в корень папки с точными именами."""
    for ext in file_extensions:
        # Загружаем файл в корень папки (без добавления имени папки в путь)
        file_key = f"{folder}{group_id}{ext}".replace(f"{folder}", "")  # Убираем префикс папки
        try:
            s3.put_object(Bucket=B2_BUCKET_NAME, Key=f"{folder}{file_key}", Body=b"mock content")
...
def main():
    ...
    empty_folders = []
    for folder in folders:
        group_id = folder.rstrip("/")  # Получаем ID папки (444, 555, 666)
        upload_mock_files(folder, f"20250110-{group_id}")
