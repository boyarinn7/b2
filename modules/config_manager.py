...
class ConfigManager:
    ...
    def get(self, key, default=None):
        ...
        defaults = {
            ...
            'FILE_PATHS.archive_folder': 'archive/',  # Добавляем дефолт для archive_folder
        }

        # Проверяем переменные окружения
        env_key = key.replace('.', '_').upper()
        env_value = os.getenv(env_key)
        if env_value:  # Изменено: проверяем, что env_value не пустое
            logging.info(f"🔑 Значение для ключа '{key}' взято из переменной окружения: {env_value}")
            return env_value

        # Если переменной окружения нет или она пустая, используем дефолт
        if key in defaults:
            logging.info(
