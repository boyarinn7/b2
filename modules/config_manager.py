...
class ConfigManager:
    ...
    def get(self, key, default=None):
        ...
        # Значения по умолчанию для случаев, когда секреты или переменные не заданы
        defaults = {
            'FILE_PATHS.log_folder': 'logs',
            'FILE_PATHS.archive_folder': 'archive/',
            'FILE_PATHS.folder_444': '444/',
            'FILE_PATHS.folder_555': '555/',
            'FILE_PATHS.folder_666': '666/',
        }

        # Проверяем переменные окружения
        env_key = key.replace('.', '_').upper()
        env_value = os.getenv(env_key)
        if env_value:  # Проверяем, что env_value не пустое (не None и не "")
            logging.info(f"🔑 Значение для ключа '{key}' взято из переменной окружения: {env_value}")
            return env_value

        # Если переменной окружения нет или она пустая, используем дефолт из defaults
        if key in defaults:
            logging.info(
                f"ℹ️ Ключ '{key}' не задан или пуст в переменных окружения, используется дефолт: {defaults[key]}")
            return defaults[key]

        # Проверяем config.json (fallback для обратной совместимости)
        keys = key.split('.')
        value = self.config_data
        try:
            for k in keys:
                if k not in value or value[k] is None:
                    logging.warning(f"⚠️ Ключ '{key}' не найден в конфигурации или равен None. Возвращается: {default}")
                    return default
                value = value[k]
            logging.info(f"🔧 Значение для ключа '{key}' взято из config.json: {value}")
            return value
        except (KeyError, TypeError) as e:
            logging.warning(f"⚠️ Ключ '{key}' не найден в конфигурации и переменных окружения. Возвращается: {default}")
            return default
        except Exception as e:
