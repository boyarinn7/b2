...
class ConfigManager:
    ...
    def get(self, key, default=None):
        """Получение значения по ключу из конфигурации или переменных окружения."""
        # Сначала проверяем переменные окружения для ключей B2
        env_key = key.replace('.',
                              '_').upper()  # Преобразуем ключ в формат переменной окружения (API_KEYS.b2.access_key -> API_KEYS_B2_ACCESS_KEY)
        env_value = os.getenv(env_key)
        if env_value is not None:
            logging.info(f"🔑 Значение для ключа '{key}' взято из переменной окружения: {env_value}")
            return env_value

        # Если переменной окружения нет, ищем в config.json
        keys = key.split('.')
        ...
        try:
            for k in keys:
                if k not in value:
                    logging.warning(
                        f"⚠️ Ключ '{key}' не найден в конфигурации. Используется значение по умолчанию: {default}")
                    return default
                value = value[k]
            return value
        except Exception as e:
            logging.error(f"❌ Ошибка при получении ключа '{key}': {e}")
            return default
