...
def load_b2_json(s3_client, bucket_name, remote_path, local_temp_path, default_value=None):
    ...
    try:
        ...
        return load_json_config(local_temp_path)
...
def save_b2_json(s3_client, bucket_name, remote_path, local_temp_path, data):
    ...
    try:
        ...
        logger.info(f"Данные успешно сохранены в {remote_path} в B2: {json.dumps(data, ensure_ascii=False)[:100]}...")
        return True
...
# --- ИСПРАВЛЕННАЯ ФУНКЦИЯ upload_to_b2 ---
